...
def main():
  ...
  identity_R = np.diag([1, 1, 1])
  identity_t = np.zeros((3, ))
  e_im2.SetPose(identity_R, identity_t)
  ...
  for pose in possible_relative_poses:
    ...
    temp_points3D, _, _ = TriangulatePoints(K, e_im1, e_im2, e_matches)
    if temp_points3D.shape[0] > npts_correct_pose:
      correct_pose = pose
      npts_correct_pose = temp_points3D.shape[0]

  # TODO
  # Set the image poses in the images (image.SetPose(...))
  e_im1.SetPose(correct_pose[0], correct_pose[1])
